...
BOOL ret;
   ...
   m_vScrollBarStations.SetScrollRange(0, MAX_SCROLLPAGES, FALSE);
...
void CMOD_simDlg::SetupListViewControl(const DWORD cols)
{
   ...
   m_listCtrlData.SetupColumns(LONG (cols+1), !IsClone());    // Made NOT 2014-01-02 by DL because bits were labeled backwards.
...
DWORD animationType;
   ...
   if (!m_registration.CheckRegistrationKey(m_registeredUser, m_registeredKey))
      CSplashWnd::SetWaitCount(1); // splash screen stays up longer if U not registered
...
void CMOD_simDlg::OnGetMinMaxInfo( MINMAXINFO FAR* lpMMI )
{
	if (IsWindows7OrGreater())			// Added to change for Win7 2015-01-02 by DL
	    lpMMI->ptMinTrackSize.x = 900;  //  Revised 2015-01-08 & 2015-07-16 by DL
	else
