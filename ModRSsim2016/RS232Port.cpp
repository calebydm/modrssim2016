		 ...
		 {                                                 // 2015-01-25 by DL to empty noise buffer
            ...
            if (m_SaveDebugToFile)                         // Section Added on 2015-02-02 by DL to send to Log File
            {
               ...
               if (m_CommsTimeShow)
               {
                  SYSTEMTIME sysTime;
                  GetLocalTime(&sysTime);
                  entry.Format("%02d:%02d:%02d.%03d Timeout on Message Receive. Discarding Data.\n",
                     sysTime.wHour, sysTime.wMinute, sysTime.wSecond,
                  sysTime.wMilliseconds);
               }
               else
                  entry = "Timeout on Message Receive. Discarding Data.\n";
               SaveDebugInfo(entry);
            }                                           // End of Section Added 2015-02-01 by DL to sent to Log File
		 }                                              // 2015-01-25 by DL to empty noise buffer
