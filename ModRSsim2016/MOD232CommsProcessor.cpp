...
static BYTE EthernetHeadder[4]= {
   ...
   //ameASCII = FALSE;   // Deleted 2015-Dec-19 by DL because it is not needed
...
BOOL MBUSError = FALSE;
      ...
      )
   {
      ...
      if ((numBytesInReq > m_PDUSize) && (modMsg.functionCode != MOD_READ_DIAGNOSTIC)) // Added MOD_READ_DIAGNOSTIC check 2015-Dec-03 by DL
      {
         MBUSError = TRUE;
   ...
   if (!MBUSError)
   {
	  ...
	  if (modMsg.address == 0)  // 08 Diagnostic Functiod Code 8 with Subfunction Code 00
	  {
		 ...
		 *(WORD*)pDataPortion = SwapBytes(modMsg.byteCount); // This is the 2-byte Query Data field to be echoed back here
		 ...
		 deb.Format("Reapond to Function Code 8, Subfunction 00 with Echo with %d bytes.\n", 8);
		 OutputDebugString(deb);
         RSDataMessage(deb); 
         break;
...
//                     pDataPortion = responseModMsg.dataPtr; //Get offset to fill in data
      ...
      case MOD_READ_SLAVEID:
		 ...
		 *pDataPortion = 0;   // Bitmapped Error Code
         ...
         pDataPortion ++;
		 // Print Debugging Information
		 deb.Format("Reapond to Function Code 17 with %d bytes.\n", 14);
		 OutputDebugString(deb);
         RSDataMessage(deb); 
         break;
      }
...
//   RSDataMessage(".\n");          // Commented out on 2015-01-17 by DL. Added extra lines to Comms debug.
